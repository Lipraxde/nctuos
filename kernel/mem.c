...
mem_init(void)
{
	...
	// Your code goes here:
	// Lab6 map kernel stack in 'mem_init_mp'
...
mem_init_mp(void)
{
	...
	int i;
	for (i = 0; i < NCPU; ++i) {
		uint32_t ks = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
		boot_map_region(kern_pgdir, ks - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W);
	}
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	*pde = (uintptr_t)ret | PTE_P | PTE_W;
	return KADDR(ret + PTX(va));
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (!pte) {
		// page_decref(pp) may cause page_free, but pp is allocated by
		// caller
		pp->pp_ref -= 1;
		return -E_NO_MEM;
	}
